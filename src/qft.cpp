...
void qft(std::vector<std::string> registerq){
    std::vector<std::string> qubits = parseQregisterVector(registerq);
...
// Quantum Fourier transform over 2^n, where n is the length of the input quantum register, approximated to k-qubit accuracy, with single qubit control
