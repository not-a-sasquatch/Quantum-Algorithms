...
void div_s(std::vector<std::string> registerq, std::vector<std::string> registerN, std::string registerAncilla, std::vector<std::string> registerQuotient, std::vector<std::string> registerModulus){
    ...
    std::string ancillaAdder = ancillas[1];
    ...
    toffoli(controls, modulus[modulus.size()-1]);
    x(ancilla1);
    cQTC(quotient[quotient.size()-1], dataQuotient);
    cQTC(modulus[modulus.size()-1], dataModulus);
    // Add sign to regq
    swap(regq[regq.size()-1], ancilla0);
    // Add sign to regN
    cQTC(ancilla1, dataRegN);
...
// Signed integers, LSB first
// Modified from algorithm in https://arxiv.org/pdf/1801.01081
void cdiv_s(std::vector<std::string> registerq, std::vector<std::string> registerN, std::string registerAncilla, std::vector<std::string> registerQuotient, std::vector<std::string> registerModulus, std::string control){
    int negDivisor = 0;
    if (N == 0){
        std::cout << "Beep boop! divval_s error - N must be non-zero.\n";
    } else if(N < 0){
    ...
    cx(control, ancilla0);
    toffoli(controls, {ancillaAdder}, quotient[quotient.size()-1]);
    toffoli(controls, {ancillaAdder}, modulus[modulus.size()-1]); 
    cx(control, ancilla0);
    cx(control, ancilla1);
    toffoli(controls, {ancillaAdder}, quotient[quotient.size()-1]);
    toffoli(controls, {ancillaAdder}, modulus[modulus.size()-1]);
    ...
    cQTC(flag, dataQuotient);
    toffoli({quotient[quotient.size()-1], control}, flag);
    toffoli({modulus[modulus.size()-1], control}, flag);
    cQTC(flag, dataModulus);
    toffoli({modulus[modulus.size()-1], control}, flag);
    // Add sign to regq
    cswap(control, regq[regq.size()-1], ancilla0);
    // Add sign to regN
...
// Signed integers, LSB first
// Modified from algorithm in https://arxiv.org/pdf/1801.01081
void divval_s(std::vector<std::string> registerq, int N, std::string ancilla, std::vector<std::string> registerQuotient, std::vector<std::string> registerModulus){
    int negDivisor = 0;
    if (N == 0){
        std::cout << "Beep boop! divval_s error - N must be non-zero.\n";
    } else if(N < 0){
    ...
    swap(result[result.size()-1], ancilla);
    // Take two's complement if regq negative
    std::vector<std::string> dataRegq(result.begin(), result.end() - 1);
    cQTC(ancilla, dataRegq);
    ...
    for(int i = m-1; i>=0; i--){
        ...
        subQFTval(subRegq, binArr);
        // Controlled re-addition
        //std::vector<int> binArr2 = intToBinArray(multValue, n+i);
        std::vector<std::string> subReg2(result.begin(), result.end() - (m - i));
        caddQFTval(subReg2, subRegq[subRegq.size() - 1], binArr);
    } 
    // Correct sign bit
    if(negDivisor){
        x(ancilla);
        swap(quotient[quotient.size()-1], ancilla);
    } else {
        swap(quotient[quotient.size()-1], ancilla);
    }
    std::vector<std::string> dataQuotient(quotient.begin()+n, quotient.end()-2);
    cQTC(dataQuotient, quotient[quotient.size()-1]);
...
void divvalIP_s(std::vector<std::string> registerq, int N, std::string ancilla){
    ...
    for(int i = m-1; i>=0; i--){
        ...
        // Flip sign bit to make addition into subtraction
        subQFTval(subRegq, binArrayN);
        // Controlled re-addition
        //std::vector<int> binArr2 = intToBinArray(multValue, n+i);
        std::vector<std::string> subReg2(regq.begin(), regq.end() - (m - i));
        caddQFTval(subReg2, subRegq[subRegq.size() - 1], binArrayN);
    } 
    // Correct sign bit
    if(negDivisor){
        x(ancilla);
...
void uncomputeQuotientval_s(std::vector<std::string> registerq, std::vector<std::string> registerQuotient, std::vector<std::string> registerModulus, int N, std::vector<std::string> ancilla){
    ...
    /////////////////////
    // quotient*N
    multiplyIPqcMAC(quotient, binArrayN, ancilla);
...
void uncomputeQuotient_s(std::vector<std::string> registerq, std::vector<std::string> registerN, std::vector<std::string> registerQuotient, std::vector<std::string> registerModulus, std::vector<std::string> ancillaRegister){
    ...
    /////////////////////
    // quotient*N
    multiplyIPMAC(quotient, regN, ancillas);
    // quotient*N + modulus (input mod N) = input
    addQFT(quotient, modulus);
    // subtract regq from quotient
    subQFT(quotient, regq);
...
void uncomputeModulusval_s(std::vector<std::string> registerq, std::vector<std::string> registerQuotient, std::vector<std::string> registerModulus, int N, std::vector<std::string> ancillaRegister){
    ...
    mulMACval(quotient, binArrayN, ancillaMult, ancillaResult);
...
// Signed integers, LSB first
void cuncomputeModulus_s(std::vector<std::string> registerq, std::vector<std::string> registerN, std::vector<std::string> registerQuotient, std::vector<std::string> registerModulus, std::vector<std::string> ancillaRegister, std::string control){
    std::vector<std::string> regq = parseQregisterVector(registerq);
    std::vector<std::string> regN = parseQregisterVector(registerN);
    std::vector<std::string> quotient = parseQregisterVector(registerQuotient);
    std::vector<std::string> modulus = parseQregisterVector(registerModulus);
...
// Signed integers, LSB first
// Incomplete
void cquotient_s(std::vector<std::string> registerq, std::vector<std::string> registerN, std::vector<std::string> registerResult, std::vector<std::string> ancillaRegister, std::string control){
    std::vector<std::string> regq = parseQregisterVector(registerq);
    std::vector<std::string> regN = parseQregisterVector(registerN);
    std::vector<std::string> result = parseQregisterVector(registerResult);
    std::vector<std::string> ancillas = parseQregisterVector(registerAncilla); 
...
void quotientval_s(std::vector<std::string> registerq, std::vector<std::string> registerN, std::vector<std::string> registerResult, std::vector<std::string> ancillaRegister){
    ...
    divval_s(regq, int N, ancillasDiv, quotient, modulus);
...
void modulusval_s(std::vector<std::string> registerq, int N, std::vector<std::string> registerResult, std::vector<std::string> ancillaRegister){
    ...
    divval_s(regq, int N, ancillasDiv, quotient, modulus);
    uncomputeQuotientval_s(regq, quotient, modulus, int N, ancilasDiv);
}
